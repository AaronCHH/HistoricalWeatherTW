...
def get_weather(date, station_id, station_name):
    # 兩次URL編碼
...
def main():
    with open('station.csv', encoding='utf-8') as f:
        ...
        for each in f_csv:
            ...
            print(station_name)
            # 表頭
            headers = ["Date", "ObsTime", "StnPres", "SeaPres", "Temperature", "Tddewpoint", "RH", "WS", "WD", "WSGust",
                       "WDGust", "Precp", "PrecpHour", "SunShine", "GloblRad", "Visb"]
            ...
            for i in range((end_date - begin_date).days + 1):
                ...
                date = str(day)
                weather_dict = get_weather(date, station_id, station_name)
                if weather_dict == {}:
